...
\TDKout{Angular properties follow the same pattern.}\TDKin{Figure~\ref{fig:molecular_angles} extends the same analysis beyond local bond distances.} Nonlinear bond angles and dihedral angles are more sensitive than bond lengths to subtle changes in the electronic structure\TDKin{ and to the automated matching of nearly symmetric structures}, but the hierarchy of basis quality remains \TDKout{clear}\TDKin{visible}.

\begin{figure*}
\centering
\includegraphics[width=\textwidth]{TM_mol_2.png}
\caption{\label{fig:molecular_angles}
\TDKout{Molecular calibration beyond bond lengths. Left: relative errors for nonlinear and dihedral angles. Right: relative errors for dipole moments and average molecular polarizabilities. Response properties are more demanding than equilibrium bond lengths, but they retain the systematic basis-set improvement needed before transferring the basis to periodic verification.}\TDKin{Molecular calibration of angular observables. The distributions show relative errors for nonlinear angles and dihedral angles. These quantities are more sensitive than bond lengths to automated atom matching and near-symmetric geometries, but they retain the same systematic basis-set trend.}}
\end{figure*}
