...
Figure~\ref{fig:component_updates} reports the two optimized comparisons that define the remaining error components. The top panel compares CP2K-GTH-UZH-new with SIRIUS-GTH-UZH-new and therefore gives the Gaussian-basis contribution for the optimized settings. The bottom panel compares SIRIUS-GTH-UZH-new with SIRIUS-FP-LAPW and gives the pseudopotential contribution. \TDKin{At the level of the underlying signed equation-of-state energy differences, the superposition of the upper and lower comparisons corresponds to the remaining total CP2K-GTH-UZH-new error relative to SIRIUS-FP-LAPW shown in the bottom panel of Fig.~\ref{fig:total_before_after}; the positive $\varepsilon$ values themselves should therefore be interpreted as component norms rather than added element by element.} Read together with the total-error reduction in Fig.~\ref{fig:total_before_after}, these maps show that the improvement is not a cosmetic redistribution of errors but a targeted reduction of the components identified as limiting. The full pseudopotential before--after diagnostic and the direct pseudopotential-update map are reported in the Supplemental Material.
