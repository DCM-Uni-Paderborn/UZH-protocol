...
\TDKout{Angular properties follow the same pattern.}\TDKin{Figure~\ref{fig:molecular_response} extends the same analysis beyond local bond distances.} Nonlinear bond angles and dihedral angles are more sensitive than bond lengths to subtle changes in the electronic structure\TDKin{ and to the automated matching of nearly symmetric structures}, but the hierarchy of basis quality remains \TDKout{clear}\TDKin{visible}. Dipole moments and polarizabilities provide a stronger test because they probe the response of the density rather than only the optimized geometry. The larger variance observed for polarizabilities is consistent with the need for diffuse flexibility, but the absence of catastrophic outliers at the TZV2P level indicates that the condition-number constraint in Eq.~(\ref{eq:molopt}) is effective.
