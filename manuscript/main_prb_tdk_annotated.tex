...
\TDKin{The corresponding pseudopotential diagnostic before and after refitting is shown in Fig.~\ref{fig:pp_before_after}. Because both panels in that comparison use SIRIUS on the pseudopotential side, the CP2K Gaussian basis is absent and the GTH-UZH pseudopotential representation is tested directly. The original settings show prominent pseudopotential-limited transition-metal and main-group outliers, whereas the improved settings retain only a smaller residual set. Together with the total-error reduction in Fig.~\ref{fig:total_before_after}, this direct comparison shows that the improvement is not a cosmetic redistribution of errors but a targeted reduction of the component identified as limiting. An illustrative estimate of the residual CP2K Gaussian-basis contribution after improvement is still given in the Supplemental Material, where it is explicitly treated as a diagnostic rather than as an independent equation-of-state comparison.}

\begin{figure*}
\TDKin{\includegraphics[width=\textwidth]{periodic-table-unaries-pp-error-before-after.png}}
\caption{\label{fig:pp_before_after}
\TDKin{Pseudopotential contribution before and after applying the UZH protocol. Top: original SIRIUS-GTH-UZH settings relative to SIRIUS-FP-LAPW. Bottom: improved SIRIUS-GTH-UZH-new settings relative to SIRIUS-FP-LAPW. Because both panels use SIRIUS on the pseudopotential side, the CP2K Gaussian basis is absent and the comparison isolates the pseudopotential transferability error.}}
\end{figure*}
