...
\subsection{\TDKout{Total error of the original UZH settings}\TDKin{Total error before and after improvement}}

\TDKout{The previous text described only the original CP2K-GTH-UZH total-error map relative to the SIRIUS-FP-LAPW reference.}
\TDKin{Figure~\ref{fig:total_before_after} compares the total practical error of the original and improved CP2K-GTH-UZH settings relative to the SIRIUS-FP-LAPW reference. In the original settings, several elements exhibit $\varepsilon>1$ for at least two unary structures, including noble gases, selected transition metals, and heavier main-group elements. After applying the UZH protocol, the corresponding CP2K-GTH-UZH-new map is substantially cleaner, with the largest residual deviations confined to a smaller set of chemically interpretable cases. This before--after comparison is the error a user would observe if CP2K were treated as a black box. It is therefore the most direct measure of the protocol's practical accuracy, but by itself it does not reveal the origin of the discrepancy.}
...
\begin{figure*}
\TDKout{Previous single original-total-error map removed.}\\[0.5ex]
\TDKin{\includegraphics[width=\textwidth]{periodic-table-unaries-total-error-before-after.png}}
\TDKcomment{Replaced the single original-total-error map by a direct before--after total-error comparison.}
\caption{\label{fig:total_before_after}
\TDKin{Total equation-of-state error relative to SIRIUS-FP-LAPW, expressed with the $\varepsilon$ metric of Eq.~(\ref{eq:epsilon}). Top: original CP2K-GTH-UZH settings. Bottom: improved CP2K-GTH-UZH-new settings. Each map contains both Gaussian-basis and pseudopotential errors and therefore represents the practical production-level CP2K error before and after applying the UZH protocol.}}
\end{figure*}

\begin{table*}
\caption{\label{tab:classification}
Qualitative classification of dominant error sources in the original UZH settings. The classification is based on the pairwise comparisons in Figs.~\TDKout{the previous decomposition and original-total-error maps}\TDKin{\ref{fig:total_before_after} and \ref{fig:pp_basis_old}}.}
\begin{ruledtabular}
\begin{tabular}{lll}
...
\TDKout{The previous main text treated the residual-basis estimate and the improved total-error map as separate main-text figures.}
\TDKin{The corresponding pseudopotential diagnostic before and after refitting is reported in the Supplemental Material. Because both panels in that comparison use SIRIUS on the pseudopotential side, the CP2K Gaussian basis is absent and the GTH-UZH pseudopotential representation is tested directly. The original settings show prominent pseudopotential-limited transition-metal and main-group outliers, whereas the improved settings retain only a smaller residual set. Together with the total-error reduction in Fig.~\ref{fig:total_before_after}, this direct comparison shows that the improvement is not a cosmetic redistribution of errors but a targeted reduction of the component identified as limiting. An illustrative estimate of the residual CP2K Gaussian-basis contribution after improvement is also given in the Supplemental Material, where it is explicitly treated as a diagnostic rather than as an independent equation-of-state comparison.}
