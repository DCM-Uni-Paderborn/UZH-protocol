...
\label{eq:overlap}
\end{equation}
and, for basis level $B$, its condition number 
\begin{equation}
\kappa_B(M)=
...
\label{eq:condition}
\end{equation}
where $M$ denotes the molecule. The MOLOPT optimization target balances accuracy and numerical stability, i.e.
\begin{equation}
\Omega =
...
\times \exp\left[-\left(\alpha^{\mathrm{pp}} r\right)^2\right],\TDKout{
\qquad
\alpha^{\mathrm{pp}}=\frac{1}{\sqrt{2}r_{\mathrm{loc}}^{\mathrm{pp}}}}
\end{multline}
\TDKin{with $\alpha^{\mathrm{pp}}=1/(\sqrt{2}r_{\mathrm{loc}}^{\mathrm{pp}})$,} and the nonlocal part is written as
\begin{equation}
