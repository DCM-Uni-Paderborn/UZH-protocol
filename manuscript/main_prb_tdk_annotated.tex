...
Nonlinear bond angles and dihedral angles extend the same analysis beyond local bond distances. They are more sensitive than bond lengths to subtle changes in the electronic structure and to the automated matching of nearly symmetric structures, but the hierarchy of basis quality remains visible; \TDKout{the corresponding distributions are reported in the Supplemental Material}\TDKin{Sec. II of the Supplemental Material reports the full angle and dihedral-error distributions}.

The response-property tests complete the molecular calibration. Dipole moments follow the same systematic basis-set trend \TDKout{and are reported in the Supplemental Material}\TDKin{with the full distribution given in Sec. II of the Supplemental Material}. Figure~\ref{fig:molecular_polarizabilities} shows the average molecular polarizability, the most demanding response observable in this set. The larger variance observed for polarizabilities is consistent with the need for diffuse flexibility, but the absence of catastrophic outliers at the TZV2P level indicates that the condition-number constraint in Eq.~(\ref{eq:molopt}) is effective. \TDKin{The supplemental bond-class and reference-code comparisons in Sec. II further support this choice: they show that the TZV2P improvement is not confined to the aggregate bond-length distribution and that the molecular calibration is not dominated by a single reference-code mismatch.}
