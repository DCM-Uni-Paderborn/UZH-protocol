...
\langle p_j^{lm}|\mathbf r'\rangle \TDKout{.}\TDKin{,}
\end{equation}
...
The results are organized as a diagnostic sequence that follows the order of the approximations in the protocol. Section~IV.A identifies the smallest converged UZH-MOLOPT basis set suitable for production GPW calculations while retaining numerical stability. Section~IV.B then quantifies the total error of the original CP2K-GTH-UZH settings before optimization. Section~IV.C separates this total error into \TDKout{a Gaussian-basis contribution followed by a pseudopotential contribution}\TDKin{Gaussian-basis and pseudopotential contributions} by inserting the SIRIUS-GTH-UZH reference between CP2K-GTH-UZH and SIRIUS-FP-LAPW. Finally, Sec.~IV.D evaluates the optimized UZH settings in the same order and shows whether the targeted basis-set and pseudopotential revisions reduce the dominant outliers.
...
Distribution of relative errors for average molecular polarizabilities. Polarizabilities are the most demanding molecular response test in this set because they require sufficient diffuse flexibility; \TDKout{TPSS is omitted because this property was not available for meta-generalized-gradient calculations in CP2K}\TDKin{meta-generalized-gradient data are omitted because this property was not available for the corresponding CP2K calculations}.}
\end{figure}
...
Figure~\ref{fig:component_updates} reports the two optimized comparisons that define the remaining error components. The top panel compares CP2K-GTH-UZH-new with SIRIUS-GTH-UZH-new and therefore gives the Gaussian-basis contribution for the optimized settings. The bottom panel compares SIRIUS-GTH-UZH-new with SIRIUS-FP-LAPW and gives the pseudopotential contribution. \TDKin{At the level of the underlying signed equation-of-state energy differences, the superposition of the upper and lower comparisons corresponds to the remaining total CP2K-GTH-UZH-new error relative to SIRIUS-FP-LAPW shown in the bottom panel of Fig.~\ref{fig:total_before_after}; the positive $\varepsilon$ values themselves should therefore be interpreted as component norms rather than added element by element.} Read together with the total-error reduction in Fig.~\ref{fig:total_before_after}, these maps show that the improvement is not a cosmetic redistribution of errors but a targeted reduction of the components identified as limiting. \TDKout{The full pseudopotential before--after diagnostic and the direct pseudopotential-update map are reported in the Supplemental Material.}\TDKin{The Supplemental Material relates the residual pseudopotential map to the direct pseudopotential-update map, making clear which refits reduce the all-electron discrepancy and which elements retain a pseudopotential-limited contribution.}
