...
\end{equation}
\TDKout{where $\nu$ labels contractions that share a compact primitive exponent set $\{\alpha_{A l p}\}$ and differ through the coefficients $c_{A l\nu p}$.}\TDKin{with $r_A=|\mathbf r-\mathbf R_A|$, where $m=-l,\ldots,l$ is the magnetic quantum number and $\nu$ labels contractions that share a compact primitive exponent set $\{\alpha_{A l p}\}$ and differ through the coefficients $c_{A l\nu p}$.} Valence and polarization flexibility are therefore introduced mainly through the contraction pattern, while the same primitive pool can be kept sufficiently compact for condensed-phase calculations.

\TDKout{The numerical stability of a candidate basis is measured with the molecular overlap matrix,}\TDKin{The numerical stability of a candidate basis is measured with the molecular overlap matrix $S_{\mu\nu}^{(M)}$ and, for basis level $B$, its condition number $\kappa_B(M)$,}
\begin{equation}
...
\end{equation}
\TDKout{where $\kappa_B(M)$ is the condition number for basis level $B$ and molecule $M$.}\TDKin{where $M$ denotes the molecule.} \TDKout{The MOLOPT optimization target balances accuracy and numerical stability,}\TDKin{The MOLOPT optimization target balances accuracy and numerical stability, i.e.,}
\begin{equation}
\Omega =
