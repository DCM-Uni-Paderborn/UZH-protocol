...
\begin{figure}
\centering
\TDKout{Previous enlarged two-column raster version removed.}\\[0.5ex]
\TDKin{\includegraphics[width=\linewidth]{TM_mol_polarizabilities_singlecol.png}}
\TDKcomment{Set Fig.~3 back to a single-column float and replaced the previous enlarged raster by a more strongly cropped and sharpened single-column version.}
...
\DIFaddend Third, some heavy elements show mixed behavior. In these cases both pairwise comparisons are non-negligible, and the improvement strategy must combine \TDKout{pseudopotential refitting with basis-set extension or reoptimization}\TDKin{basis-set extension or reoptimization with pseudopotential refitting}.
...
Error decomposition for the original UZH settings. Left: CP2K-GTH-UZH relative to SIRIUS-GTH-UZH, which isolates the Gaussian-basis error. Right: SIRIUS-GTH-UZH relative to SIRIUS-FP-LAPW, which isolates the pseudopotential error.}
\end{figure*}
...
Figures~\ref{fig:improved_pp} and \ref{fig:improved_total} \TDKout{show the resulting total error of CP2K-GTH-UZH-new relative to SIRIUS-FP-LAPW and the residual decomposition, again with the basis-set contribution preceding the pseudopotential contribution.}\TDKin{show the improved settings in two steps. Figure~\ref{fig:improved_pp} compares an illustrative estimate of the remaining CP2K Gaussian-basis contribution with the residual pseudopotential error. The basis-set estimate was obtained as the positive difference between the final total-error map in Fig.~\ref{fig:improved_total} and the residual pseudopotential map shown to its right; it should therefore be read as a diagnostic guide rather than as a direct CP2K-GTH-UZH-new/SIRIUS-GTH-UZH-new equation-of-state comparison. Figure~\ref{fig:improved_total} gives the resulting total error of CP2K-GTH-UZH-new relative to SIRIUS-FP-LAPW. The CP2K-GTH-UZH-new versus original SIRIUS-GTH-UZH comparison is not used as a residual basis-set map because it would mix the remaining Gaussian-basis contribution with the change from the original to the improved pseudopotential. The direct comparison between the new and original SIRIUS-GTH-UZH pseudopotential settings is therefore reported only as a diagnostic in the Supplemental Material.} The improved settings reduce the dominant outliers of the original protocol and make the remaining discrepancies more interpretable. In particular, noble-gas and Ba errors are reduced by basis-set changes, while the transition-metal cases respond to pseudopotential refitting. The resulting UZH protocol is therefore not a fixed table of parameters but a reproducible recipe for diagnosing, optimizing, and validating CP2K settings.

\begin{figure*}
\centering
\TDKout{Single-panel residual pseudopotential map replaced by the two-panel comparison below.}\\[0.5ex]
\TDKout{\makebox[0.49\textwidth][l]{\textbf{(a)}}\hfill
\makebox[0.49\textwidth][l]{\textbf{(b)}}\\[-0.5ex]}
\TDKin{\includegraphics[width=0.49\textwidth]{periodic-table-unaries-CP2K-UZH-new-vs-SIRIUS-UZH-new-epsilon-estimated-padded.png}\hfill
\includegraphics[width=0.49\textwidth]{periodic-table-unaries-SIRIUS-UZH-new-vs-SIRIUS-LAPW-epsilon-padded-strongbox.png}}
\TDKcomment{Removed the panel letters, added top padding to both Fig.~6 raster files, and strengthened the blue structure-box border in the right panel so that the upper line remains visible after scaling.}
\caption{\label{fig:improved_pp}
\TDKout{Verification of the improved UZH settings. Top left: total CP2K-GTH-UZH-new error relative to SIRIUS-FP-LAPW. Top right: residual CP2K Gaussian-basis error. Bottom left: residual pseudopotential error after improvement. Bottom right: direct comparison of new and original SIRIUS-GTH-UZH pseudopotential settings.}\TDKout{(a) Illustrative estimate of the residual CP2K Gaussian-basis contribution after improvement, constructed as $\max[\epsilon_{\mathrm{total}}-\epsilon_{\mathrm{pp}},0]$ from Fig.~\ref{fig:improved_total} and panel (b). (b) Residual pseudopotential error for the improved UZH settings, obtained from SIRIUS-GTH-UZH-new relative to the SIRIUS-FP-LAPW reference.}\TDKin{Left: illustrative estimate of the residual CP2K Gaussian-basis contribution after improvement, constructed as $\max[\epsilon_{\mathrm{total}}-\epsilon_{\mathrm{pp}},0]$ from Fig.~\ref{fig:improved_total} and the residual pseudopotential map. Right: residual pseudopotential error for the improved UZH settings, obtained from SIRIUS-GTH-UZH-new relative to the SIRIUS-FP-LAPW reference.}}
\end{figure*}

\begin{figure*}
\centering
\includegraphics[width=0.72\textwidth]{periodic-table-unaries-CP2K-UZH-new-vs-SIRIUS-LAPW-epsilon.png}
\caption{\label{fig:improved_total}
\TDKin{Total equation-of-state error of the improved CP2K-GTH-UZH-new settings relative to SIRIUS-FP-LAPW. This comparison is the final production-level accuracy test and contains the remaining CP2K Gaussian-basis contribution together with the residual pseudopotential error shown in Fig.~\ref{fig:improved_pp}.}}
\end{figure*}

\section{Implications for CP2K verification}

The UZH protocol turns CP2K verification into a diagnostic and constructive workflow. The comparison with SIRIUS-FP-LAPW gives the total accuracy relevant for production calculations. The intermediate SIRIUS-GTH-UZH calculation tells whether the Gaussian basis or the pseudopotential is responsible. \TDKout{The CP2K ATOM code then supplies the internal machinery for improvement:}\TDKin{The newly developed CP2K ATOM machinery then supplies the internal route for improvement:} \DIFaddbegin \DIFadd{basis optimization with condition-number control, }\DIFaddend scalar-relativistic atomic references, \DIFaddbegin \DIFadd{and }\DIFaddend GTH parameter fitting\DIFdelbegin \DIFdel{, and basis optimization with condition-number control}\DIFdelend . This is particularly valuable for users who need reliable calculations across molecules and solids, because the protocol connects molecular transferability to periodic equation-of-state precision.
